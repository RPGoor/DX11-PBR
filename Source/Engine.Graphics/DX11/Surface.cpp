...
void Surface::PutPixel(unsigned int x, unsigned int y, Color c) conexcept
{
    assert(x >= 0);
    assert(y >= 0);
    assert(x < width);
...
Surface::Color Surface::GetPixel(unsigned int x, unsigned int y) const conexcept
{
    assert(x >= 0);
    assert(y >= 0);
    assert(x < width);
...
Surface Surface::FromFile(const std::string& name)
{
...

    {
        ...
        if (bitmap.GetLastStatus() != Gdiplus::Status::Ok)
        {
            ...
            throw DX11SurfaceException(__LINE__, __FILE__, ss.str());
        }
...
void Surface::Copy(const Surface& src) conexcept
{
